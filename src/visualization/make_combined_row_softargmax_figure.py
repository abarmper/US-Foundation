"""
Merges the two separate figures (best-per-task row + HC soft-argmax
heatmaps) into ONE combined figure with two rows and a single caption:
  Row (a): best (lowest-MRE) validation case for four representative tasks
           spanning different clinical domains.
  Row (b): soft-argmax probability heatmaps for one HC keypoint case.

Does not touch or regenerate the original separate figures
(fig_best_per_task_row.*, softargmax_heatmaps/fig_softargmax_HC.*) --
this is purely an additional, alternative combined asset so the two-figure
version can be restored at any time by re-pointing main.tex.

Usage:
  CUDA_VISIBLE_DEVICES=1 python make_combined_row_softargmax_figure.py \
      --checkpoint ../runs/v3_true_hrnet_softargmax/checkpoints/best_teacher_model.pth \
      --manifest figures/qualitative_best/manifest.json \
      --row_tasks AOP HC A4C IVC \
      --softargmax_task HC \
      --out_stem figures/fig_combined_row_softargmax
"""
import os
import sys
import json
import argparse
import numpy as np
import torch
import torch.nn.functional as F
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
import cv2
import albumentations as A
from mpl_toolkits.axes_grid1.inset_locator import inset_axes, mark_inset

sys.path.append(os.path.dirname(os.path.abspath(__file__)))  # for "common"
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # src/
if PROJECT_ROOT not in sys.path:
    sys.path.append(PROJECT_ROOT)
from common import load_image_rgb, INFERENCE_TRANSFORM, GT_JSON_PATH, remove_padding_and_scale
from gubiometry.models import UnifiedBiometryModel

os.environ.setdefault("TORCH_HOME", os.path.join(os.path.expanduser("~"), ".cache", "torch_gu_biometry"))
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
CANVAS = 518


def get_soft_argmax_coords(logits, temperature=10.0):
    B, K, H, W = logits.shape
    heatmaps = F.softmax(logits.view(B, K, -1) * temperature, dim=-1)
    grid_y, grid_x = torch.meshgrid(torch.arange(H, device=logits.device),
                                     torch.arange(W, device=logits.device), indexing="ij")
    grid_x, grid_y = grid_x.flatten().float(), grid_y.flatten().float()
    pred_x = torch.sum(heatmaps * grid_x, dim=-1) * (CANVAS / W)
    pred_y = torch.sum(heatmaps * grid_y, dim=-1) * (CANVAS / H)
    return pred_x, pred_y


def get_soft_argmax_coords_and_maps(logits, temperature=10.0):
    B, K, H, W = logits.shape
    probs = F.softmax(logits.view(B, K, -1) * temperature, dim=-1).view(B, K, H, W)
    grid_y, grid_x = torch.meshgrid(torch.arange(H, device=logits.device),
                                     torch.arange(W, device=logits.device), indexing="ij")
    grid_x, grid_y = grid_x.flatten().float(), grid_y.flatten().float()
    flat = probs.view(B, K, -1)
    pred_x = torch.sum(flat * grid_x, dim=-1) * (CANVAS / W)
    pred_y = torch.sum(flat * grid_y, dim=-1) * (CANVAS / H)
    return pred_x, pred_y, probs


def main(args):
    with open(args.manifest) as f:
        manifest = json.load(f)
    with open(GT_JSON_PATH) as f:
        gt_records = json.load(f)
    gt_by_key = {}
    for rec in gt_records:
        fname = os.path.basename(rec["image_path"])
        gt_by_key[(rec["task_id"], fname)] = rec["ground_truth_points_pixels"]

    print("Loading model...")
    model = UnifiedBiometryModel(
        freeze_encoder=True, dropout_p=0.3,
        unfreeze_last_n_blocks=args.unfreeze_last_n_blocks,
        neck_branch_width=tuple(args.neck_branch_width),
        shared_head=args.shared_head,
    ).to(device)
    print("Load result:", model.load_state_dict(torch.load(args.checkpoint, map_location=device), strict=True))
    model.eval()

    n_cols = max(len(args.row_tasks), 4)
    fig, axes = plt.subplots(2, n_cols, figsize=(4.0 * n_cols, 8.6))

    # ---- Row (a): best-per-task overlays ----
    with torch.no_grad():
        for ax, task_id in zip(axes[0], args.row_tasks):
            best = manifest[task_id]["selected"][0]
            abs_path = best["image_path"]
            fname = os.path.basename(abs_path)
            gt_pts_flat = gt_by_key[(task_id, fname)]

            img_rgb = load_image_rgb(abs_path)
            orig_h, orig_w = img_rgb.shape[:2]
            tensor_img = INFERENCE_TRANSFORM(image=img_rgb)["image"].unsqueeze(0).to(device)
            logits = model.forward_phase2(tensor_img, task_id)
            pred_x_518, pred_y_518 = get_soft_argmax_coords(logits, args.softargmax_temp)

            gt_pts = np.asarray(gt_pts_flat, dtype=float).reshape(-1, 2)
            n_kp = min(logits.shape[1], gt_pts.shape[0])
            pred_pts = np.array([
                remove_padding_and_scale(pred_x_518[0, k].item(), pred_y_518[0, k].item(), orig_h, orig_w)
                for k in range(n_kp)
            ])
            visible = np.all(gt_pts[:n_kp] >= 0, axis=1)

            ax.imshow(img_rgb)
            ax.scatter(gt_pts[visible, 0], gt_pts[visible, 1], c="lime", marker="o", s=55,
                       edgecolors="black", linewidths=0.7, zorder=3)
            ax.scatter(pred_pts[visible, 0], pred_pts[visible, 1], c="red", marker="x", s=55,
                       linewidths=1.8, zorder=3)
            for k in range(n_kp):
                if visible[k]:
                    ax.plot([gt_pts[k, 0], pred_pts[k, 0]], [gt_pts[k, 1], pred_pts[k, 1]],
                            c="yellow", linewidth=0.9, alpha=0.9, zorder=2)
            ax.set_title(f"{task_id}  (MRE {best['mre_px']:.2f}px)", fontsize=12, fontweight="bold")
            ax.axis("off")
        for j in range(len(args.row_tasks), n_cols):
            axes[0, j].axis("off")

    # ---- Row (b): soft-argmax heatmaps for one task ----
    task_id = args.softargmax_task
    best = manifest[task_id]["selected"][0]
    abs_path = best["image_path"]
    fname = os.path.basename(abs_path)
    gt_pts = np.asarray(gt_by_key[(task_id, fname)], dtype=float).reshape(-1, 2)

    img_rgb = load_image_rgb(abs_path)
    orig_h, orig_w = img_rgb.shape[:2]
    tensor_img = INFERENCE_TRANSFORM(image=img_rgb)["image"].unsqueeze(0).to(device)
    display_transform = A.Compose([
        A.LongestMaxSize(max_size=CANVAS),
        A.PadIfNeeded(min_height=CANVAS, min_width=CANVAS, border_mode=cv2.BORDER_CONSTANT, fill=0),
    ])
    canvas_rgb = display_transform(image=img_rgb)["image"]

    with torch.no_grad():
        logits = model.forward_phase2(tensor_img, task_id)
        pred_x_518, pred_y_518, probs = get_soft_argmax_coords_and_maps(logits, args.softargmax_temp)
    probs = probs[0].cpu().numpy()
    n_kp = min(logits.shape[1], gt_pts.shape[0])

    scale = CANVAS / max(orig_h, orig_w)
    new_h, new_w = int(orig_h * scale), int(orig_w * scale)
    pad_top, pad_left = (CANVAS - new_h) // 2, (CANVAS - new_w) // 2

    ZOOM_HALF_WIN, DISPLAY_GAMMA = 22, 0.3
    for k in range(min(n_kp, n_cols)):
        ax = axes[1, k]
        heat = probs[k]
        heat_up = cv2.resize(heat, (CANVAS, CANVAS), interpolation=cv2.INTER_CUBIC)
        heat_up = np.clip(heat_up, 0, None)
        heat_up = heat_up / (heat_up.max() + 1e-8)
        heat_up = heat_up ** DISPLAY_GAMMA

        gt_x_518, gt_y_518 = gt_pts[k, 0] * scale + pad_left, gt_pts[k, 1] * scale + pad_top
        pred_x, pred_y = pred_x_518[0, k].item(), pred_y_518[0, k].item()

        ax.imshow(canvas_rgb)
        ax.imshow(heat_up, cmap="inferno", alpha=0.55, extent=(0, CANVAS, CANVAS, 0))
        ax.scatter([gt_x_518], [gt_y_518], c="lime", marker="o", s=70,
                   edgecolors="black", linewidths=0.8, zorder=4, label="Ground truth")
        ax.scatter([pred_x], [pred_y], c="cyan", marker="x", s=70,
                   linewidths=2.2, zorder=4, label="Soft-argmax prediction")
        ax.set_title(f"Keypoint {k} (peak prob {heat.max():.3f})", fontsize=10)
        ax.set_xlim(0, CANVAS); ax.set_ylim(CANVAS, 0)
        ax.axis("off")

        x0, x1 = pred_x - ZOOM_HALF_WIN, pred_x + ZOOM_HALF_WIN
        y0, y1 = pred_y - ZOOM_HALF_WIN, pred_y + ZOOM_HALF_WIN
        axins = inset_axes(ax, width="42%", height="42%", loc="lower left", borderpad=0.6)
        axins.imshow(canvas_rgb)
        axins.imshow(heat_up, cmap="inferno", alpha=0.6, extent=(0, CANVAS, CANVAS, 0))
        axins.scatter([gt_x_518], [gt_y_518], c="lime", marker="o", s=140, edgecolors="black", linewidths=1.0, zorder=4)
        axins.scatter([pred_x], [pred_y], c="cyan", marker="x", s=140, linewidths=2.6, zorder=4)
        axins.set_xlim(x0, x1); axins.set_ylim(y1, y0)
        axins.set_xticks([]); axins.set_yticks([])
        for spine in axins.spines.values():
            spine.set_edgecolor("white"); spine.set_linewidth(1.5)
        mark_inset(ax, axins, loc1=1, loc2=3, fc="none", ec="white", linewidth=0.8, alpha=0.7)
    for j in range(n_kp, n_cols):
        axes[1, j].axis("off")

    gt_patch = mpatches.Patch(color="lime", label="Ground truth")
    pred_patch = mpatches.Patch(color="red", label="Prediction (best-per-task row)")
    line_patch = mpatches.Patch(color="yellow", label="GT–Pred error vector")
    cyan_patch = mpatches.Patch(color="cyan", label="Soft-argmax prediction (heatmap row)")
    fig.legend(handles=[gt_patch, pred_patch, line_patch, cyan_patch], loc="lower center", ncol=2, fontsize=10)
    plt.tight_layout(rect=[0, 0.06, 1, 1])

    out_dir = os.path.dirname(args.out_stem) or "."
    os.makedirs(out_dir, exist_ok=True)
    for ext in ["png", "pdf", "eps"]:
        out_path = f"{args.out_stem}.{ext}"
        kwargs = {"dpi": 300} if ext == "png" else {}
        plt.savefig(out_path, format=ext, bbox_inches="tight", **kwargs)
        print(f"Saved {out_path}")
    plt.close(fig)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--checkpoint", type=str, required=True)
    parser.add_argument("--manifest", type=str, required=True)
    parser.add_argument("--row_tasks", type=str, nargs="+", default=["AOP", "HC", "A4C", "IVC"])
    parser.add_argument("--softargmax_task", type=str, default="HC")
    parser.add_argument("--unfreeze_last_n_blocks", type=int, default=4)
    parser.add_argument("--neck_branch_width", type=int, nargs=3, default=[128, 96, 64])
    parser.add_argument("--shared_head", action="store_true")
    parser.add_argument("--softargmax_temp", type=float, default=10.0)
    parser.add_argument("--out_stem", type=str, default="figures/fig_combined_row_softargmax")
    args = parser.parse_args()
    main(args)
