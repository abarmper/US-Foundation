...
| `model.neck.decoder` | `hrnet` | `simple`, `dpt` | Post-backbone decoder (orthogonal to `input_mode`). `simple` = ViTPose-style deconv upsampler (2× ConvTranspose → 148), replacing HRNet's multi-branch fusion; keeps the soft-argmax heads/loss unchanged. With `input_mode=single` it uses the last-layer grid (`configs/abl_ep20_simplehead.yaml`, ~5.3M neck); with `input_mode=multilevel` it **concatenates** the `feature_layers` depths channel-wise (`configs/abl_ep20_simplehead_ml.yaml`, ~18M neck — the concat analogue of hrnet-multilevel's sum). Motivated by ViTPose (strong backbone → minimal decoder). `dpt` = DPT/RefineNet-style top-down progressive fusion: each of the 4 `feature_layers` depths gets its own projection and is fused coarsest-to-finest through residual conv units, rather than `simple`'s undifferentiated concat or `hrnet`'s bidirectional exchange; **multilevel-only** (`configs/abl_ep20_dptdecoder_ml.yaml`, ~24M neck — heavier than both `hrnet` and `simple`-multilevel, testing whether depth-aware fusion beats concat now that `simple`-concat is the current best-blend result). |
